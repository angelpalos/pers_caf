...
const DB_NAME = process.env.DB_NAME || 'sistema_gamma';
...
app.get('/', (req,res) => {
    if (req.session.loggedin == true) {
        ...
        req.getConnection((err, conn) => {
            ...
            }else if (userData[0].Id_rol == "Entrega"){
                res.render('home',{name: req.session.name,email: req.session.email, rol_Entrega:userData[0].Id_rol});
            }
            })
        })   
